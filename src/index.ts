...
function findIntersectInRegistry(intersectees, registry, getTag) {
  return registry.reduce((acc, [setTags, func]) => {
...
function defaultIntersectHandle({ intersectees }) {
  ...
  return jsc
    .suchthat(allIntersectees, tuple =>
      some(tuple, x => guardEvery(intersectees, x))
    )
    .smap(tuple => find(tuple, x => guardEvery(intersectees, x)), identity);
}
...
const INTERSECT_REGISTRY = [
  [
    new Set(["partial", "record"]),
    ({ intersectees }) =>
      jsc
        .tuple(intersectees.map(intersect => makeJsverifyArbitrary(intersect)))
        .smap(
          tupleOfTypes =>
            tupleOfTypes.reduce((acc, item) => Object.assign(acc, item)),
    ...
    ({ intersectees }) => {
      ...
      return jsc.bless({
        ...
        generator: allAltArb.generator.flatmap(tuple => {
          const onlyIntersectees = tuple.filter(x =>
            guardEvery(intersectees, x)
          );
...
const REGISTRY = {
  ...
  intersect: ({ intersectees }) => {
    const handler =
      findIntersectInRegistry(intersectees, INTERSECT_REGISTRY, x => x.tag) ||
      defaultIntersectHandle;
  ...
  partial: ({ fields }) => {
    ...
    return jsc.record(spec).smap(rec => {
      const recWithoutEmpty = Object.assign({}, rec);
...
export function makeJsverifyArbitrary<T extends Reflect>(
  type: T
): jsc.Arbitrary<any> {
...
export function addTypeToRegistry<T extends Reflect>(
  tag: string,
  generator: (x: T) => jsc.Arbitrary<any>
): void {
  CUSTOM_REGISTRY[tag] = generator;
}

export function addTypeToIntersectRegistry<T extends Reflect>(
  tags: string[],
  generator: (x: T) => jsc.Arbitrary<any>
): void {
  CUSTOM_INTERSECT_REGISTRY.push([new Set(tags), generator]);
}
...
export function generateAndCheck<T extends Reflect>(rt: T, opts?: jsc.Options) {
  return () => {
    ...
    jsc.assert(
      jsc.forall(arbitrary, function arbitraryIsChecked(anything) {
        rt.check(anything);
        return true;
      }),
      opts
