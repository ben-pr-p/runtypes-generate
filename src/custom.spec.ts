...
function generatorContains(rt: Constraint<any, { tag: "contains" }>) {
  ...
  return makeJsverifyArbitrary(underlying).smap(
    coll => {
      const res = contains(
        coll,
        args,
        ...
        (list, res) => {
          ...
          const idxs = list.length
            ? ((jsc.sampler(jsc.elements(range(list.length)))(
                howMuch
              ) as any) as number[])
            : range(howMuch);
          const elements = jsc.sampler(makeJsverifyArbitrary(res.item))(
            howMuch
          ) as any[];
...
describe("FamilyObject", () => {
  ...
  const NotSpouse = MemberWithRole(
    Union(Literal("sibling"), Literal("child"), Literal("parent"), Void)
  );
  const Member = Spouse.Or(NotSpouse);
  const FamilyWithTypeAndMember = (type, countSpouse) =>
    Record({
      type,
      members: Array(Member).withConstraint(members => {
        const foundSpouse = members.filter(m => Spouse.guard(m));
        return foundSpouse.length == countSpouse;
      })
    });

  const FamilyWithSpouse = FamilyWithTypeAndMember(Literal("espoused"), 1);
  const FamilyWithoutSpouse = FamilyWithTypeAndMember(
    Union(Literal("single"), Literal("common_law_marriage"), Void),
    0
  );

  const membersWithSpouse = Array(Member).withConstraint(members => {
    const foundSpouse = members.filter(m => Spouse.guard(m));
    return foundSpouse.length > 0;
  });
